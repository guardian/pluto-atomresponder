...
class TestMediaAtomToken(django.test.TestCase):
    ...
    def test_get_token(self):
        ...
        :return:
        """
        from atomresponder.media_atom import get_token
        result = get_token("https://my-server/my-path","SomeKindaLongSecret", override_time=datetime(2018,3,1,18,13,14))
        print(result)
...
class TestMediaAtomResend(django.test.TestCase):
    def test_atom_resend_normal(self):
        ...
        :return:
        """
        with patch("atomresponder.media_atom.get_token", return_value=('HMAC fake-code', 'Thu, 01 Mar 2018 18:13:14 GMT')) as mock_get_token:
            with patch("requests.post", return_value=MockResponse(200, """{"status":"ok"}""", {"Content-Type":"application/json"})) as mock_post:
                from atomresponder.media_atom import request_atom_resend, HttpError
