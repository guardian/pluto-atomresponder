...
class TestAPIViews(APITestCase):
    ...
    def test_notification_normal(self):
        from atomresponder.job_notification import JobNotification
        ...
        with patch('atomresponder.notification.process_notification') as mock_process:
            with patch('atomresponder.job_notification.JobNotification', return_value=mock_notification) as mock_notification_constructor:
                ...
                mock_notification_constructor.assert_called_once_with(data.encode("UTF-8"))
    ...
    def test_notification_xmlerror(self):
        ...
        with patch('atomresponder.notification.process_notification') as mock_process:
            with patch('atomresponder.job_notification.JobNotification', side_effect=LxmlError("Nobody expects the Spanish Inqusition!")) as mock_notification_constructor:
                response = client.post(reverse_lazy("import_notification"),data=data,content_type='application/xml')
