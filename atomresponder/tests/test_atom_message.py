...
class TestAtomMessage(unittest2.TestCase):
    ...
    def test_parse_and_get(self):
        from atomresponder.atom_message import AtomMessage
        with open(self.TEST_DATA_FILE) as f:
            a = AtomMessage(f.read())

        self.assertEqual(a.description,"Leicester City will take on Atl\xe9tico Madrid in the last eight of the Champions League after the draw for the quarter-final stage of this season's competition took place on Friday. Dortmund will play Monaco, Bayern Munich meet Real Madrid and Barcelona take on Juventus")
        self.assertEqual(a.category, "News")
    ...
    def test_poster_images(self):
        ...
        image_list = [x for x in a.posterImages]
